...
const getPagingData = (data, page, limit) => {
    ...
    const niveis = data.rows;
    const currentPage = page;
    const totalPages = Math.ceil(totalItems / limit);

    return { totalItems, niveis, totalPages, currentPage };
};
...
exports.findAll = async (req, res) => {
    ...
    try {
        let niveis = await Nivel.findAndCountAll({ where: condition, offset })
        
        // map default response to desired response data structure
        res.status(200).json({
            success: true,
            totalItems: niveis.count,
            niveis: niveis.rows,
        ...
        res.status(500).json({
            success: false, msg: err.message || "Some error occurred while retrieving the levels."
        })
...
exports.findOne = async (req, res) => {
    try {
        let nivel = await Nivel.findByPk(req.params.nivelID)

         if (nivel === null)
            res.status(404).json({
                success: false, msg: `Cannot find any level with ID ${req.params.nivelID}.`
            });
        else
            res.json({ success: true, nivel: nivel });
    }
    catch (err) {
        res.status(500).json({
            success: false, msg: `Error retrieving level with ID ${req.params.nivelID}.`
        });
    };
};
